...
This section introduces the ways to run the minCounter and the input format of parameters :
    ...
    input parameters: 
    	A) max_loop: when inserting a new element, if the times of kick out elements equals to max_loop, we think there are a infinite loop.
    	B) init_rate: when initiate the hash table, we set the size of hash table equals to the size of dataset  multiplied by init_rate.
	C) file_name: file_name is the name of dataset file.
